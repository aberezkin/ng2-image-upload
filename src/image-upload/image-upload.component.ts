...
export class ImageUploadComponent implements OnInit {
      ...
      FirebaseServiceUsing: boolean = false;
      ...
      @Input() firebaseUsing: boolean;
      @Input() firebaseFolderName: string;
      @Input() firebaseImageNameRandom: boolean;
      ...
      ngOnInit() {
            ...
            if (this.firebaseUsing) { this.FirebaseServiceUsing = true; }; // firebase input sends config json.
      }
            ...
            for (let i = 0; i < filesToUploadNum; i++) {
                  ...
                  reader.addEventListener('load', (event: any) => {
                        const fileHolder: FileHolder = new FileHolder(event.target.result, beforeUploadResult.file);
                         ...
                         if (this.FirebaseServiceUsing) {
                            this.uploadSingleFileWithFS(fileHolder);
                        } else {
      ...
      private uploadSingleFileWithFS(fileHolder: FileHolder) {
            if (this.FirebaseServiceUsing) {
                  this.pendingFilesCounter++;
                  fileHolder.pending = true;

                  this.FirebaseService
                        .postImageToBucket(fileHolder.file, this.firebaseFolderName,this.firebaseImageNameRandom )
                        .then(
                        response => this.onResponse(response, fileHolder),
                        error => {
                              this.onResponse(error, fileHolder);
